...
def parse(opt_path, is_train=True):
    ...
    if is_train:
        experiments_root = osp.join(opt['path']['root'], 'experiments', os.path.basename(opt_path).split('.')[0]) # experiments_root = osp.join(opt['path']['root'], 'experiments', opt['name'])
        ...
        # if 'debug' in os.path.basename(opt_path).split('.')[0]: # if 'debug' in opt['name']:
        #     opt['train']['val_freq'] = 8
        #     opt['logger']['print_freq'] = 1
        #     opt['logger']['save_checkpoint_freq'] = 8
    else:  # test
        results_root = osp.join(opt['path']['root'], 'results', os.path.basename(opt_path).split('.')[0]) # results_root = osp.join(opt['path']['root'], 'results', opt['name'])
